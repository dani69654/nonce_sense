...
export const WORKERS: Workers[] = JSON.parse(process.env.WORKERS || '[]');
...
export const formatPercentage = ({ value, difficulty }: { value: number; difficulty: number }) => {
    if (!difficulty || !value || isNaN(value)) return '';
    const percentage = ((value / difficulty) * 100).toFixed(4);
